...
export function createAppError(error: unknown, context?: string): AppError {
  ...
  if (errorMessage.includes('insufficient funds') || errorMessage.includes('Insufficient')) {
    return {
      ...
      userMessage: 'Insufficient balance. Please check your wallet balance and try again.',
      retryable: false,
  ...
  if (errorMessage.includes('Invalid') && errorMessage.includes('address')) {
    return {
      ...
      message: errorMessage,
      userMessage: 'Invalid address. Please check the recipient address and try again.',
      retryable: false,
      originalError: error instanceof Error ? error : undefined,
    };
  }
  ...
  if (
    ...
    return {
      ...
      message: errorMessage,
      userMessage: 'RPC endpoint error. The system will automatically retry with a different endpoint.',
      retryable: true,
      originalError: error instanceof Error ? error : undefined,
    };
  }
  ...
  if (
    ...
    return {
      ...
      message: errorMessage,
      userMessage: 'Transaction failed. Please try again or check the transaction details.',
      retryable: true,
      originalError: error instanceof Error ? error : undefined,
    };
  }

  if (errorMessage.includes('not initialized') || errorMessage.includes('initialize')) {
    return {
