...
(function () {
  ...
  window.addEventListener("message", (event) => {
    if (
      event.source !== window ||
      !event.data ||
      event.data.source !== "veil-provider"
    ) {
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!pendingRequests || typeof pendingRequests.set !== "function") {
          console.error(
            "[Veil Provider] pendingRequests is invalid:",
            pendingRequests
          );
  ...
  class SimplePublicKey {
    ...
    equals(other) {
      ...
      const otherKey =
        typeof other.toBase58 === "function" ? other.toBase58() : String(other);
    ...
    const provider = {
      ...
      signTransaction: async (transaction) => {
        ...
        if (transaction instanceof Uint8Array) {
          ...
          throw new Error(
            "Invalid transaction format. Expected VersionedTransaction or Uint8Array."
          );
        }
